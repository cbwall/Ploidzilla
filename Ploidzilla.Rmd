...
# model
anova(lmer(prey.cap.h.cm2~treatment*ploidy*size+ (1|tank) + (1|genotype), data=feed.df), type=3) # no effect
ranova(lmer(prey.cap.h.cm2~treatment*ploidy+ (1|tank) + (1|genotype), data=feed.df)) # no random effect best

Anova(lm(prey.cap.h.cm2~treatment+ploidy+size, data=feed.df), type=2) # no effect
anova(lm(prey.cap.h.cm2~size, data=feed.df)) # no effect
...
feed.plot<-ggplot(feed.df, aes(treatment, prey.cap.h.cm2, fill=ploidy)) +
 geom_point(aes(color=ploidy), alpha=0.7, position=position_jitterdodge())+
  geom_boxplot(outlier.shape = NA, alpha=0.7)+
  ylab(expression(paste("prey.cap h-1 cm-2")))+
  xlab("Treatment")+ theme_bw()+
  ylim(0, 170)+
  scale_fill_manual(values=c("goldenrod", "plum4")) +
  scale_color_manual(values=c("goldenrod", "plum4"))+
