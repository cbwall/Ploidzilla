...
### Symbiont density
```{r}
# symbiont counts from microscopy and wax dipping
...
symb.plot<-ggplot(symb.df, aes(ploidy, cells.cm2, fill=ploidy,)) +
  ...
  xlab("Ploidy State")+ theme_bw() + guides(fill="none", color="none") + ggtitle("In situ corals")
...
SA.reduc<- SA %>% select(plug.ID, surface.area.cm2) # select column needed
...
bw.SA$mg.CaCO3.cm2.d<- bw.SA$growth.corrected.mg.d / bw.SA$surface.area.cm2
bw.SA<-na.omit(bw.SA) # remove any NAs
bw.SA<-bw.SA[!(bw.SA$plug.ID=="64" | bw.SA$plug.ID=="902"),] # growth low

# format metadata
make.fac<-c("treatment", "plug.ID", "ploidy", "tank", "genotype")
bw.SA[make.fac]<-lapply(bw.SA[make.fac], factor) # make all these factors

# model
anova(lmer(mg.CaCO3.cm2.d~ploidy*treatment+ (1|tank) + (1|genotype), data=bw.SA), type=2) # no effect
...
growth.plot<-ggplot(bw.SA, aes(treatment, mg.CaCO3.cm2.d, fill=ploidy)) +
  geom_point(aes(color=ploidy), alpha=0.7, position=position_jitterdodge())+
...
SA<-read.csv("data/SA.R.csv")
SA.T1<- SA[(SA$time.point=="T1"),] # only T1 data

# merge in blastate
cyto.SA<- merge(cyto.blast, SA.T1, by = "plug.ID", all.x = TRUE)

# make cells/cm2
cyto.SA$symb.dens<- (cyto.SA$symb.cells.ml*cyto.SA$blastate.ml)/cyto.SA$surface.area.cm2
cyto.SA$symb.dens<-round(cyto.SA$symb.dens, 0)
cyto.SA<- cyto.SA[!(cyto.SA$plug.ID=="60"),] # SA is 0, to be fixed
...
ranova(lmer(symb.dens~treatment*ploidy+ (1|tank) + (1|genotype), data=cyto.SA)) # genotype signif
...
symb.plot.tank<-ggplot(cyto.SA, aes(treatment, symb.dens, fill=ploidy)) +
  ...
  xlab("Treatment")+ theme_bw()+
  scale_y_continuous(limits= c(0, 600000), labels = scales::scientific) +
  scale_fill_manual(values=c("goldenrod", "plum4")) +
  scale_color_manual(values=c("goldenrod", "plum4")) + ggtitle("Tank corals")
...
write.csv(cyto.SA, "output/cyto.df.csv")
...
### Feeding trials
```{r}
feed<-read.csv("data/feeding.csv")
feed.simple<- feed %>% select(treatment, tank, genotype, ploidy, plug.ID, 
                          prey.capture.h, surface.area.cm2, prey.cap.h.cm2)

# Convert specific columns by name 
feed.simple <- feed.simple %>% mutate(across(c(prey.capture.h, surface.area.cm2, prey.cap.h.cm2), as.numeric))
feed.simple<-feed.simple[(feed.simple$prey.cap.h.cm2>=1),] # remove any negative values

feed.df<-na.omit(feed.simple) # remove NAs

# add size class category and order levels
feed.df$size<-ifelse(feed.df$surface.area.cm2< 20, "small", "medium")
feed.df$size<-factor(feed.df$size, levels=c("small", "medium"))
sum<-aggregate(ploidy~size, data=feed.df, FUN=length)

### inspects some points that may be culled
feed.df<-feed.df[!(feed.df$prey.cap.h.cm2>150 | feed.df$prey.cap.h.cm2<15),] # remove any high outliers
#feed.x<-feed.df[(feed.df$size=="medium" & feed.df$prey.cap.h.cm2>90),] # remove any high outliers


### FEEDING BY SIZE EFFECT
anova(lm(prey.capture.h~surface.area.cm2, data=feed.df)) # size matters
ggplot(feed.df, aes(x=surface.area.cm2, y=prey.capture.h))+
  geom_point()+
  geom_smooth(method="glm", method.args=list(family=gaussian(link="log")))
       

# model
anova(lmer(prey.cap.h.cm2~treatment*ploidy*size+ (1|tank) + (1|genotype), data=feed.df), type=3) # size effect
ranova(lmer(prey.cap.h.cm2~treatment*ploidy+ (1|tank) + (1|genotype), data=feed.df)) # no random effects

Anova(lm(prey.cap.h.cm2~treatment*ploidy*size, data=feed.df), type=2) # ploidy and size effect
anova(lm(prey.cap.h.cm2~size, data=feed.df)) # size effect

feed.mod<-lm(prey.cap.h.cm2~treatment+ploidy+size, data=feed.df)
posthoc.feed<-emmeans(feed.mod, ~ploidy)
multcomp::cld(posthoc.feed, Letters=LETTERS)

##### growth boxplot 
feed.plot<-ggplot(feed.df, aes(treatment, prey.cap.h.cm2, fill=ploidy)) +
 geom_point(aes(color=ploidy), alpha=0.7, position=position_jitterdodge())+
  geom_boxplot(outlier.shape = NA, alpha=0.7)+
  ylab(expression(paste("prey.cap cm-2 h-1 ")))+
  xlab("Treatment")+ theme_bw()+
  ylim(0, 170)+
  scale_fill_manual(values=c("goldenrod", "plum4")) +
  scale_color_manual(values=c("goldenrod", "plum4"))+
...
feed.plot
dev.copy(pdf, "figures/feeding.pdf", width = 5, height = 8)
dev.off()
